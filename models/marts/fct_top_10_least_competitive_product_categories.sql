...
overpriced_products_ranking as (
    ...
    select 
        *,
        dense_rank() over(order by overpriced_count desc) as rank -- taking ties into account, so if multiple products have the same competitive count, they will receive the same rank, and the next rank will be skipped accordingly. This allows for a more accurate representation of the competitive landscape among the products.
    from overpriced_products 
    where overpriced_count > 0 -- filtering to include only products that have at least one instance of being 'overpriced' in terms of price competitiveness, ensuring that the analysis focuses on products that are actively overpriced in the market.   
